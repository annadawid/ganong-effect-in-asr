...
## Version 32:  Fixed unmathced 'if' statement in line 669
## Version 33:  The output documentation now correctly reports that there is 
##		  no change in aspiration intensity if you declined to have 
##		  aspiration intensity covary along the VOT continuum. 
###########################################################################
###########################################################################
	...
	# Print aspiration intensities
		...
		if scale_to_equal_RMS$ == "no"
        	...
        	if shape_vowel_envelope_onset == 1
        		print 'newline$'###############################################'newline$'
        		print Vowel onset intensity ramping'newline$'
		endif
endproc
...
procedure set_variables_at_startup
	...
	############################################
	# COVARIANCE OF VOT AND ASPIRATION INTENSITY
	# if the user does nto want to covary aspiration intensity along with VOT,
	# then set the covariance range to be zero
		if covary_aspiration_intensity == 0
			aspiration_intensity_range = 0
		endif
